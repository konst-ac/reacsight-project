...
int n_leds = 7;
Adafruit_NeoPixel strip_1 = Adafruit_NeoPixel(n_leds, 2, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel strip_8 = Adafruit_NeoPixel(n_leds, 5, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel strip_3 = Adafruit_NeoPixel(n_leds, 8, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel strip_4 = Adafruit_NeoPixel(n_leds, 11, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel strip_6 = Adafruit_NeoPixel(n_leds, 25, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel strip_5 = Adafruit_NeoPixel(n_leds, 31, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel strip_2 = Adafruit_NeoPixel(n_leds, 37, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel strip_7 = Adafruit_NeoPixel(n_leds, 49, NEO_GRB + NEO_KHZ800);
...
void loop() {
  ...
  while (Serial.available() < 2);
   ...
   byte target_reactor = Serial.read();
    if ((target_reactor < 9) || (target_reactor > 16)) {
      Serial.flush();
    }
    else {
      byte desired_intensity = Serial.read();
      // apply to LEDs
      set_intensity_strip(target_reactor - 8, desired_intensity);
      show_strip(target_reactor - 8);
    }
