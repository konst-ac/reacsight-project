            ...
            if not self.waiting_for_guava_data and not self.waiting_for_guava_data_sec:
                ...
                if self.sec_schedule:
                    ...
                    if next_sec_time < time():
                        ...
                        self.manager.send_ot2_request(f'transfer/200/sampling_3/{self.sec_count}/prep_plate/{self.sec_count+6}/')
                        # from the mCer column of the prep_plate to Beads columns of the prep_plate
                        self.manager.send_ot2_request(f'transfer/200/prep_plate/{self.sec_count+6}/prep_plate/{self.sec_count}/')
                        ...
                        self.manager.send_ot2_request(f'transfer/200/prep_plate/{self.sec_count}/mag_plate/{self.sec_count}/')
                        # we wash 3 times the beads
                        for i in range(1,4):
                            self.manager.send_ot2_request('engage/magdeck')
                            self.manager.send_ot2_request(f'transfer/200/mag_plate/{self.sec_count}/printed_trash/1/') 
                            ...
                            self.manager.send_ot2_request(f'transfer/200/tbs_tank/{self.sec_count}/mag_plate/{self.sec_count}/')
