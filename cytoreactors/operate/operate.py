...
class Session():
    ...
    def __init__(self, fake_OT2=False, virtual_mode=False, automated_beads = True):
        ...
        self.sec_count = 1
        self.start_incubation_time = 0
        self.waiting_for_guava_data_sec = False
        self.should_wash_row2 = False 
        self.waiting_for_1h_incubation = False
    ...
    def reset_all_sec_state(self):
        ...
        while input('WIPE WITH TISSUE THE SAMPLING LINES OUTPUT, AND RESTART THE FLASK APP OF THE ROBOT (INSTRUCTIONS AT THE BEGINNING OF THE NOTEBOOK). then wait for the robot movement and enter ok\n') != 'ok':
            pass
        self.manager.send_ot2_request('move_to/sampling_metal_3/12')
        while input('fine tune the plate position and enter ok\n') != 'ok':
            pass
        self.manager.send_ot2_request('move_to/sampling_metal_3/1')
        while input('fine tune the plate position and enter ok\n') != 'ok':
            pass
        self.manager.send_ot2_request('gotobin')
        if should_restart:
    ...
    def loop(self):
        try:
            ...
            ## To give the priority to the secretion sample after 1h of incubation
            if self.waiting_for_1h_incubation and not self.waiting_for_guava_data and not self.should_wash:
                if self.start_incubation_time + 5*60 < time(): #3600:
                    # the 1h of incubation has been performed (NB: to be tested with less incubation time in dry testes)
                    self.waiting_for_1h_incubation = False
                    self.start_incubation_time = 0
                    ...
                    self.manager.send_guava_request('prepare_sec/A12_B12_C12_D12_E12_F12_G12_H12/1000')
                    # Sara added the stirring and decrease the number of events
                    ...
                    if entering : 
                        ...
                        if next_sampling_time < time():
                            ...
                            for reactor_id, program in self.programs_with_cytometry.items():
                                ...
                                if not (not self.low_volume_no_sampling_safety or vol_estim.empty or vol_estim['volume_estimation_mL'].iloc[-1] > 12):
            ...
            ## should we sample to the guava and start acquire ?
            if not self.waiting_for_guava_data_sec:
                # check if we passed the time of the next sampling
                if self.sec_schedule:
                    ...
                    if next_sec_time < time():
                        ...
                        self.manager.send_ot2_request(f'mix_beads_sample/100/{self.sec_count}/{self.sec_count+6}')
                        
                        self.waiting_for_1h_incubation = True
                        self.start_incubation_time = time()
