...
class Session():
    ...
    def __init__(self, fake_OT2=False, virtual_mode=False):
    ...
    def reset_all_sampling_state(self):
        ...
        if should_restart:
            self.start()
        return 'Done sampling state reset... Means should be two clean sampling plates, two full reservoirs'
    ...
    def loop(self):
        try:
            ...
            if not self.waiting_for_guava_data:
            ...
            if self.should_wash_row2:
                ...
                # do the wash
                for i in range(3):
                    count = len(self.secs)+6
                    self.manager.send_ot2_request(f'wash_row_2/{count}')# find a way for iteration 
            ...
            ## should we sample to the guava and start acquire ?
            if not self.waiting_for_guava_data and not self.waiting_for_guava_data_sec:
                # check if we passed the time of the next sampling
                if self.sec_schedule:
                    ...
                    if next_sec_time < time():
                        ...
                        sec_count = len(self.secs) 
                        self.manager.send_ot2_request(f'transfer/200/sampling/{self.ot2_col}/prep_plate/{sec_count+6}/')
                        # from the mCer column of the prep_plate to Beads columns of the prep_plate
                        self.manager.send_ot2_request(f'transfer/200/prep_plate/{sec_count+6}/prep_plate/{sec_count}/')
                        # from the beads column of the prep_plate to mag_plate
                        self.manager.send_ot2_request('disengage/magdeck') # we place the liquid with the bead with a disengage magdeck, as it seems more appropriate 
                        self.manager.send_ot2_request(f'transfer/200/prep_plate/{sec_count}/mag_plate/{sec_count}/')
                        # we wash 3 times the beads
                        for i in range(1,4):
                            self.manager.send_ot2_request('engage/magdeck')
                            self.manager.send_ot2_request(f'transfer/200/mag_plate/{sec_count}/printed_trash/1/') 
                            # do not write trash but printed trash
                            #if you write column 3 for printed trash it will trigger an error 
                            self.manager.send_ot2_request('disengage/magdeck')
                            self.manager.send_ot2_request(f'transfer/200/tbs_tank/{sec_count}/mag_plate/{sec_count}/')
                        ...
                        self.manager.send_ot2_request(f'transfer/200/mag_plate/{sec_count}/cytoplate/2/')
