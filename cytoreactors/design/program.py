...
class Program:
    ...
    def load(self, ignore_new=False):
        ...
        if os.path.isfile(f'{self.output_path}/beads.csv'):
            self.beads = pd.read_csv(f'{self.output_path}/beads.csv')
    @property
    ...
    def plot_all_beads(self, ch_fl='GRN-B-HLin',ax=None,ylim=(1,30000),delta_i_per_hr=1000,log_scale=True,filter_str=None,down_sampling=None):
        if ax is None:
            f,ax = plt.subplots(figsize=(14,8), ncols=1, nrows=1)
        if filter_str is not None:
            beads = self.beads.query(filter_str).reset_index()
        else:
            beads = self.beads
        n_tp = len(beads.groupby('time_s'))
        colors = plt.cm.jet(np.linspace(0,1,n_tp))
        i_tp = 0
        delta_i = 0
        cum_i = 0
        tp_prev = beads['time_s'].iloc[0]
        for tp,d in beads.groupby('time_s'):
            delta_i += (tp-tp_prev) / 3600 * delta_i_per_hr
            if down_sampling is not None:
                d = d.iloc[-down_sampling:]
            ax.scatter(np.arange(len(d)) + cum_i + delta_i, d[ch_fl], s=0.1, c=np.array([colors[i_tp]]))
